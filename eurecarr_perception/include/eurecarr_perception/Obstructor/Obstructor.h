...
namespace eurecarr_perception
{
  ...
  public:
    ...
    void SelectTrack();
    void SelectFarthestTrack();
    void GetTrack();

    void GetClosestWaypoint(nav_msgs::Odometry current_pose, std::vector<float> wptOneDim, float& distance, int& current_index);
    void GetFarthestWaypoint(nav_msgs::Odometry current_pose, std::vector<float> wptOneDim, float& distance, int& current_index);
    ...
    void PublishTrackBoundary(std::vector<float> wptOneDim_in, std::vector<float> wptOneDim_out);
  ...
  private:
    ...
    ros::Publisher currentIndexPub_;
    ros::Publisher boundaryWptPub_in_;
    ros::Publisher boundaryWptPub_out_;
