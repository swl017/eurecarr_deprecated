...
class PoseToOdom:
    ...
    def poseSubCallback(self, pose_msg):
        ...
        v_x = (x - self.last_pose_msg.pose.position.x) / dt
        v_y = (y - self.last_pose_msg.pose.position.y) / dt
        v_z = (z - self.last_pose_msg.pose.position.z) / dt
        ...
        self.roll_rate = self.roll_rate_lpf.lpf(self.roll_rate)
        self.pitch_rate = self.pitch_rate_lpf.lpf(self.pitch_rate)
        ...
        roll_rate_rate = (self.roll_rate - self.last_roll_rate) / dt
        pitch_rate_rate = (self.pitch_rate - self.last_pitch_rate) / dt
        yaw_rate_rate = (self.yaw_rate - self.last_yaw_rate) / dt

        self.accelMsg.linear.x = self.ax_lpf.lpf(self.ax)
        self.accelMsg.linear.y = self.ay_lpf.lpf(self.ay)
        self.accelMsg.linear.z = self.az_lpf.lpf(self.az)
        self.accelMsg.angular.x = self.roll_rate_rate_lpf.lpf(roll_rate_rate)
        self.accelMsg.angular.y = self.pitch_rate_rate_lpf.lpf(pitch_rate_rate)
        self.accelMsg.angular.z = self.yaw_rate_rate_lpf.lpf(yaw_rate_rate)
        ...
        self.pose_odom_pub.publish(self.cur_pose_odom)
        self.accel_pub_all.publish(self.accelMsg)


    def poseToTF(self, msg):
        self.t.header.stamp = msg.header.stamp
        self.t.header.frame_id = "odom"
        self.t.child_frame_id = self.prefix + "base_link"
...
def main():
    rospy.init_node("poseToOdom", anonymous=True)
    prefix = rospy.get_namespace()
    if (len(prefix) > 2):
        prefix = prefix[1:-1] + "/"
    print("prefix = ", prefix[0:-1])
    poseToOdom = PoseToOdom()
    poseToOdom.prefix = prefix
    rate = rospy.Rate(50) #  Hz
    while not rospy.is_shutdown():
        poseToOdom.speed_pub.publish(poseToOdom.cur_speed)
        poseToOdom.slip_pub.publish(poseToOdom.cur_slip)
        # poseToOdom.pose_odom_pub.publish(poseToOdom.cur_pose_odom)
        poseToOdom.yaw_pub.publish(poseToOdom.yaw)
        poseToOdom.yaw_rate_pub.publish(poseToOdom.yaw_rate)
        poseToOdom.accel_pub_x.publish(poseToOdom.ax)
        poseToOdom.accel_pub_y.publish(poseToOdom.ay)
        poseToOdom.accel_pub_z.publish(poseToOdom.az)
        # poseToOdom.accel_pub_all.publish(poseToOdom.accelMsg)
        # poseToOdom.pose_odom_pub.publish(poseToOdom.cur_pose_odom)

        # print( 'roll pitch yaw = ', poseToOdom.roll, poseToOdom.pitch, poseToOdom.yaw )
