...
namespace eurecarr_perception {
  ...
  Obstructor::Obstructor(ros::NodeHandle nh)
  {
    ...
    boundaryWptPub_in_ = nh_.advertise<nav_msgs::Path>("waypoint/boundary/in", 1);
    boundaryWptPub_out_ = nh_.advertise<nav_msgs::Path>("waypoint/boundary/out", 1);
    refWptLocalPub_ = nh_.advertise<nav_msgs::Path>("waypoint/reference/local", 1);
    currentIndexPub_ = nh_.advertise<std_msgs::Int64>("waypoint/currentIndex", 1);
    
    OpenReferenceWaypoint();
    std::string thisnamespace = ros::this_node::getNamespace();
    thisnamespace = thisnamespace.substr(1);
    // std::cout << "namesapce = " << thisnamespace << ", " << thisnamespace.length() << std::endl;
    if (thisnamespace.length()>=1){
        thisnamespace.push_back('/');
    }
    // std::cout << "namesapce = " << thisnamespace << ", " << thisnamespace.length() << std::endl;
    thisns_ = thisnamespace;
    ...
    void Obstructor::EgoSubCallback(nav_msgs::OdometryConstPtr pose)
  {
    ...
    // PublishTrackBoundary(track_in_, track_out_);
  ...
  void Obstructor::SelectFarthestTrack()
  {
    // TODO: make it polar
    ...
    //compare indexes on opponent FARTHEST track
    track_num = std::distance(oppo_distances.begin(), std::min_element(oppo_distances.begin(), oppo_distances.end()));
    ego_index = ego_indexes.at(track_num);
    oppo_index = oppo_indexes.at(track_num);


    // EVADE only if the opponent is closely AHEAD
    if ((oppo_index - ego_index > OBSTRUCT_INDEX_RANGE_MIN && oppo_index - ego_index < OBSTRUCT_INDEX_RANGE)
          || (waypoint_length[track_num] - ego_index + oppo_index < OBSTRUCT_INDEX_RANGE && waypoint_length[track_num] - ego_index + oppo_index > OBSTRUCT_INDEX_RANGE_MIN)){
      // selected_track_ = track_num;
      selected_track_ = track_num;
      ego_index = ego_indexes.at(selected_track_);
      // ROS_INFO("OBSTRUCTING, track = %d, o_idx = %d, e_idx = %d, dist = %f, %f, %f", selected_track_, oppo_index, ego_index,
  ...
  void Obstructor::GetTrack()
  {
    ...
    for (int i = 0; i < NUM_OF_TRACKS; i++){
      ...
      // oppo_indexes.push_back(oppo_index);
      // GetClosestWaypoint(ego_pose_, wpt_to_test, distance, ego_index);
      GetClosestWaypoint(ego_pose_, track_mid_, distance, ego_index);
      ego_distances.push_back(distance);
      ego_indexes.push_back(ego_index);
    }
  ...
  void Obstructor::GetClosestWaypoint(nav_msgs::Odometry current_pose, std::vector<float> wptOneDim, float& distance, int& current_index)
  {
    ...
    for(int i = 0; i < (int)(length/2.0); i+=2){
      x = wptOneDim.at(i);
      y = wptOneDim.at(i+1);
    ...
    std_msgs::Int64 current_index_topic;
    current_index_topic.data = current_index;
    if(current_index != 0 && firstLoop_){
      lastIndex_ = current_index;
      firstLoop_ = false;
    }
    currentIndexPub_.publish(current_index_topic);
    lastIndex_ = current_index;
    distance = min_dist;
    //
  ...
  void Obstructor::PublishSelectedTrack()
  {
    ...
    // float waypoint_length1 = waypoint_length;
    ...
    reference_waypoint.header.frame_id = thisns_ + "base_link";
    refWptLocalPub_.publish( reference_waypoint );
  }

  void Obstructor::PublishTrackBoundary(std::vector<float> boundaryWaypoints_in, std::vector<float> boundaryWaypoints_out){
    // Inner Boundary
    nav_msgs::Path boundary_waypoint; 
    geometry_msgs::PoseStamped wpt;
    wpt.header.stamp = ros::Time::now();
    wpt.header.frame_id = target_frame_.c_str();
    // wpt.header.frame_id = "odom";
    boundary_waypoint.header.stamp = wpt.header.stamp;
    boundary_waypoint.header.frame_id = wpt.header.frame_id;
    int waypoint_length = boundaryWaypoints_in.size();

    // float waypoint_length1 = std::min(20,waypoint_length);
    float waypoint_length1 = waypoint_length;
    for (int i=2; i<waypoint_length1-2; i+=2){
      wpt.pose.position.x = boundaryWaypoints_in.at(i);
      wpt.pose.position.y = boundaryWaypoints_in.at(i+1);
      boundary_waypoint.poses.push_back(wpt);
    }
    boundaryWptPub_in_.publish( boundary_waypoint );

    // Outer Boundary
    boundary_waypoint.poses.clear();
    // wpt.header.frame_id = "odom";
    waypoint_length = boundaryWaypoints_out.size();

    // waypoint_length1 = std::min(20,waypoint_length);
    waypoint_length1 = waypoint_length;
    for (int i=2; i<waypoint_length1-2; i+=2){
      wpt.pose.position.x = boundaryWaypoints_out.at(i);
      wpt.pose.position.y = boundaryWaypoints_out.at(i+1);
      boundary_waypoint.poses.push_back(wpt);
    }
    boundaryWptPub_out_.publish( boundary_waypoint );
